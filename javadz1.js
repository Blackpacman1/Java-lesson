...
g = snake.getContext('2d'),
sBody = null,
d = 1,
a = null, 
s = 25; newB(); newA(); 
snake.width = innerWidth;
snake.height = innerHeight; 
setInterval(function(){
	if (a[0] + s >= snake.width || a[1] + s >= snake.height) newA(); 
	g.clearRect(0,0,snake.width,snake.height);
g.fillStyle = "red";
	g.fillRect(...a, s, s);
	g.fillStyle = "#000";
...
sBody.forEach(function(pob, i){
		if (d == 1) if (pob.x > Math.round(snake.width / s) * s) pob.x = 0;
if (d == 2) if (pob.y > Math.round(snake.height / s) * s) pob.y = 0;
if (d == 3) if (pob.x < 0) pob.x = Math.round(snake.width / s) * s;
if (d == 4) if (pob.y < 0) pob.y = Math.round(snake.height / s) * s;
